...
export default function Cart() {
      ...
      const updated = prev
        .map((store) => ({
          ...
          productList: store.productList.filter(
            (product) => !checkedItems[product.cartIdx]
          ),
        }))
        ...
        if (!isBundleGroup) {
          ...
          products.forEach((product) => {
            if (checkedItems[product.cartIdx]) {
              totalPrice +=
                (product.productSalePrice + product.optionPrice) *
                product.quantity;
  ...
  const increaseCount = (cartIdx, productIdx, optionIdx) => {
    ...
    setGroupedCart((prev) =>
      prev.map((store) => ({
        ...store,
        productList: store.productList.map((product) =>
          product.cartIdx === cartIdx
    ...
    setOrderList((prev) =>
      prev.map((order) =>
        order.productId === productIdx && order.optionId === optionIdx
      ...
      prev
        .map((store) => ({
          ...
          productList: store.productList
            ...
            .filter((product) => product.quantity > 0), // 0이면 삭제
        }))
        .filter((store) => store.productList.length > 0)
    );
    ...
    setOrderList(
      (prev) =>
        prev
          ...
          .filter((order) => order.count > 0) // 0이면 삭제
    );
      ...
      <div className="Cart-main-div">
        ...
        <div className="cart-product-info-list">
          ...
          <div className="cart-top-selectAll-div">
            ...
            <button
              className="cart-select-delete-button font-14 medium"
              onClick={cartDeleteProducts}
            >
              선택삭제
            </button>
          </div>
          <table className="cart-table">
            ...
            <tbody>
              {groupedCart.map((store) => {
                ...
                const groupedByUnit = store.productList.reduce(
                  (acc, product) => {
                    if (!acc[product.postType]) acc[product.postType] = [];
                    acc[product.postType].push(product);
                    return acc;
                ...
                return (
                  <React.Fragment key={store.brandId}>
                      ...
                      <td colSpan={5}>
                        ...
                        >
                          <span
                            ...
                            className="font-14 medium"
                          >
                            {store.brandName}
                          </span>
                          <span>
                            묶음배송 무료 전환 기준금액 :{" "}
                            {store.freeChargeAmount}
                          </span>
                        </div>
                      </td>
                      <td>
                        <a
                          href={"/zipddak/storeInfo/" + store.brandId}
                          className="font-14 cart-store-info-a"
                        >
                          <span>업체 정보</span>{" "}
                          <i
                    ...
                    {/* 배송단위별 그룹 */}
                    {Object.entries(groupedByUnit).map(
                      ([unit, items], groupIdx, arr) => {
                          ...
                          <React.Fragment key={unit}>
                            {items.map((product, idx) => (
                              <tr
                                ...
                                style={{
                                  borderBottom:
                                    !isLastGroup && idx === items.length - 1
                                      ? "1px solid #e6e6e6"
                                      : "none",
                                }}
                              >
                                    ...
                                    <span className="font-15">
                                      {product.postType === "single"
                                        ...
                                        : (() => {
                                            ...
                                            const returnPostCharge =
                                              sum >= store.freeChargeAmount
                                                ? 0
                                                : store.basicPostCharge;
