...
export default function ProductDetail() {
  ...
  const handleOption = (optionId, type = "top") => {
    ...
    const selectedOption = productOption[targetOption]?.find(
      (o) => o.optionId === Number(optionId)
    );
    if (selectedOption) {
      const info = {
  ...
  const addOption = (type = "top") => {
    ...
    const existingIndex = orderList.findIndex(
      (item) => item.optionId === info.optionId
    );
    if (existingIndex !== -1) {
  ...
  const loadMoreReviews = async () => {
    ...
    try {
      const res = await axios.get(
        `${baseUrl}/reviews?productId=${productId}&page=${nextPage}`
      );
  ...
  const loadMoreInquiries = async () => {
    ...
    try {
      const res = await axios.get(
        `${baseUrl}/inquiries?productId=${productId}&page=${nextPage}`
      );
  ...
  useEffect(() => {
    myAxios(token, setToken)
      .get(
        `${baseUrl}/product?productId=${productId}&username=${user.username}`
      )
      .then((res) => {
        const data = res.data;
        console.log(data);
          ...
          <div className="detail-top-right">
            <div className="detail-product-info">
              ...
              <div className="detail-product-name-div">
                ...
                <button
                  onClick={() => favoriteToggle(product.productIdx)}
                  style={{ border: "none", backgroundColor: "transparent" }}
                >
                  {favorite ? (
                    <i className="bi bi-heart-fill product-like"></i>
                  ) : (
              ...
              <div className="detail-product-div-under">
                <div className="detail-price-review-div">
                  <div>
                    <div className="detail-sale-div">
                      {/* 세일 퍼센트 */}
                      <span className="detail-sale-percent">
                        {product.discount}%
                      </span>
                      {/* 정가 */}
                      <del className="detail-default-price">
                ...
                <div className="detail-select-option">
                  ...
                  <div className="detail-select-div">
                    <select
                      onChange={(e) =>
                        handleOptionChange(e.target.value, "top")
                      }
                  ...
                  <div className="detail-select-div">
                    <select
                      onChange={(e) => handleOption(e.target.value, "top")}
                      className="detail-select"
                      value={selectOptionInfo.optionId || "none"}
                    >
                      <option value="none" disabled>
                        색상
                      </option>
                  ...
                  <div className="detail-button-div">
                    <button
                      onClick={() => addOption("top")}
                      type="button"
                      className="detail-append-buy"
                    >
                      구매 추가
                    </button>
                  </div>
                    ...
                    <div key={option.optionId} className="detail-append-box">
                      ...
                      <div className="detail-append-box-bottom detail-box-son">
                        ...
                        <div className="detail-append-button">
                          <button
                            className="count-button-style"
                            onClick={() => decreaseCount(option.optionId)}
                          >
                            <i className="bi bi-dash-lg append-button-son"></i>
                          </button>
                          <span className="append-button-son">
                            {option.count}
                          </span>
                          <button
                            className="count-button-style"
                            onClick={() => increaseCount(option.optionId)}
                          >
                        ...
                        <span className="detail-order-price">
                          {(option.price * option.count).toLocaleString()}원
                        </span>
                      </div>
                    </div>
                  ))}
                ...
                <div>
                  <div className="detail-order-price-div">
                    ...
                    {/* 주문 금액 */}
                    <span className="detail-order-hard-price">
                  ...
                  <div className="font-14">
                    방법 :{" "}
                    {product.postYn ? (
                      <span className="font-14">(택배배송)</span>
                    ) : (
                      <></>
                    )}
                    {product.pickupYn ? (
                      <span className="font-14">(픽업가능)</span>
                    ) : (
                      <></>
                    )}
                  </div>
                  <div className="font-14">
                    ...
                    {product.postType === "single" ? (
                      <span className="font-14">개별배송</span>
                    ) : (
                      <span className="font-14">묶음배송</span>
                    )}
                  </div>
                  <div className="font-14">
                    배송비 : {product?.postCharge?.toLocaleString()}원
                  </div>
                  <div className="font-14">결제 : 주문 시 결제</div>
                  <div className="font-14">
                    안내 : 상품이 발송되면 송장번호를 마이페이지에서 확인하실 수
                    있습니다.
                  </div>
                </div>
                <div className="detail-order-button-div">
                  <button
                    onClick={addCart}
                    className="detail-order-button go-cart"
                  >
                    장바구니
                  </button>
                  <button
        ...
        <div className="detail-product-bottom-nav">
          {navItems.map((item) => (
            <button
              key={item.id}
              className={
                bottomSelect === item.id
                  ? "bottom-nav-select-button"
                  : "bottom-nav-button"
              }
            ...
            <div className="detail-bottom-left">
              ...
              <div ref={reviewRef} className="detail-bottom-reivew">
                <div>
                  ...
                  {/* 리뷰 수 */}
                  <span className="detail-bottom-review-count">
                    {reviewCount}
                  </span>
                </div>
                ...
                <div className="detail-bottom-review-box-div">
                  {reviews.map((review) => (
                    <div
                      key={review.reviewProductIdx}
                      className="detail-bottom-review-box"
                    >
                      <div>
                        {/* 사용자 닉네임 */}
                        <span className="detail-bottom-review-nickname">
                          {review.nickname}
                        </span>
                      </div>
                      <div>
                        <div className="detail-bottom-review-start-list-div">
                          ...
                          {/* 리뷰 작성일 */}
                          <span className="detail-bottom-review-created">
                            {review.createdate}
                          </span>
                        </div>
                      </div>
                      <img
                        className="detail-bottom-review-img"
                        src="/images/이미지테스트.png"
                      />
                ...
                <div className="productDetail-info-more-btn-div">
                  {reviewCount > reviews.length && (
                    <button
                      ...
                      className="productDetail-info-more-btn"
                    >
              ...
              <div ref={askRef} className="detail-bottom-ask">
                <div className="detail-bottom-ask-title-div">
                  <div>
                    ...
                    {/* 문의 수 */}
                    <span className="detail-bottom-ask-count">
                      {inquiryCount}
                    </span>
                  </div>
                  {/* 문의 하기 버튼 */}
                ...
                <div className="detail-bottom-ask-box-div">
                  {inquiries.map((inquiry) => (
                    <div
                      className="detail-bottom-ask-box"
                      key={inquiry.inquiryIdx}
                    >
                      <div>
                        {/* 문의 사용자 닉네임 */}
                        <span className="detail-bottom-ask-nickname">
                          {inquiry.writerNickname}
                        </span>
                        {/* 문의 날짜 */}
                        <span className="detail-bottom-ask-created">
                          {inquiry.writeAt}
                        </span>
                      </div>
                        ...
                        <tbody className="detail-bottom-ask-table">
                          <tr>
                            ...
                            <td className="detail-bottom-ask-left-padding-0">
                              {/* 문의 내용 */}
                              <span className="detail-bottom-ask-content">
                                {inquiry.content}
                              </span>
                            </td>
                          </tr>
                          <tr>
                            ...
                            <td className="detail-bottom-ask-left-padding-0">
                              {/* 자재업체 이름 */}
                              <span className="detail-bottom-ask-storeName">
                                {inquiry.brandName}
                              </span>
                              {/* 답변 날짜 */}
                              <span className="detail-bottom-ask-return-created">
                                {inquiry.answerAt}
                              </span>
                            </td>
                          </tr>
              ...
              <div ref={deliveryRef} className="detail-bottom-return-div">
                ...
                <div>
                  ...
                  <table className="detail-bottom-return-table">
                    <tbody>
                      <tr>
                        <td className="detail-bottom-return-table-first-td">
                          배송 단위
                        </td>
                        {/* 배송 단위 */}
                        <td>
                      ...
                      <tr>
                        ...
                        {/* 수령 방법 */}
                        <td>
                          {product.postYn ? <span>(택배배송)</span> : <></>}{" "}
                          {product.pickupYn ? <span>(픽업가능)</span> : <></>}
                        </td>
                      </tr>
                      <tr>
                        <td className="detail-bottom-return-table-first-td">
                ...
                <div>
                  ...
                  <div className="detail-bottom-return-second-div">
                    <div>
                      환불 신청은 <a href="#">마이페이지</a>{" "}
                      <i className="bi bi-caret-right-fill"></i>{" "}
                      <a href="#">주문 목록</a>에서 가능합니다.
                    </div>
                    <div>
                      상품 수령 후 단순 변심으로 인한 환불 시, 왕복 배송비가
                      발생할 수 있습니다.{" "}
                    </div>
                    <div>
                      환불 진행 상황은 마이페이지에서 확인하실 수 있습니다.
                    </div>
                  </div>
                </div>
                <div>
                  ...
                  <div className="detail-bottom-return-third-div">
                    <div>
                      1. 이미 설치하거나 사용한 제품은 재판매가 어렵기 때문에
                      반품 불가.
                    </div>
                    <div>
                      2. 제품의 포장이나 라벨이 훼손되어 상품 가치가 감소한
                      경우.
                    </div>
                    <div>
                      3. 배송 후 일정 기간{" "}
                      <span className="detail-bottom-return-third-div-span">
                        (예: 7일, 14일 등)
                      </span>{" "}
                      이상 지난 경우, 또는 날씨·환경으로 인해 제품 가치가 감소한
                      경우.
                    </div>
                  </div>
                </div>
            ...
            <div className="detail-bottom-right">
              <div className="detail-select-option">
                <div className="detail-select-div">
                  <select
                    onChange={(e) =>
                      handleOptionChange(e.target.value, "bottom")
                    }
                    className="detail-select2"
                    defaultValue={"none"}
                  >
                ...
                <div className="detail-select-div">
                  <select
                    onChange={(e) => handleOption(e.target.value, "bottom")}
                    className="detail-select2"
                    value={selectOptionInfo2.optionId || "none"}
                  >
                    <option value="none" disabled>
                      색상
                    </option>
                ...
                <div>
                  <div className="detail-order-price-div">
                    ...
                    <div>
                      {/* 주문 금액 */}
                      <span className="detail-order-hard-price2">
                        {totalPrice.toLocaleString()}원
                      </span>
                    </div>
                  </div>
                ...
                <div className="detail-order-button-div">
                  <button
                    onClick={() => favoriteToggle(product.productIdx)}
                    className="detail-bottom-order-heart"
                  >
                    {favorite ? (
                      <i className="bi bi-heart-fill product-like"></i>
                    ) : (
                  ...
                  <button className="detail-order-button2 go-cart">
                    장바구니
                  </button>
                  <button
                    onClick={() => navigate("/zipddak/productOrder")}
