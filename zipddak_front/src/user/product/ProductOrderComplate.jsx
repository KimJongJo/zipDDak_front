...
export default function ProductOrderComplate() {
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}/user/orderInfo?orderCode=${orderCode}`)
      .then((res) => {
        ...
        <div className="order-complate-first-table">
          ...
          <table className="order-complate-top-table margin-top-20">
            <tbody>
              ...
              {orderList.map((order) => (
                <tr
                  key={order.orderItemIdx}
                  className="order-complate-second-table-tr"
                >
                  <td>
                    <img
                      className="order-complate-product-img"
                      src={order.img}
                    />
                  </td>
                  <td>
                  ...
                  <td>
                    <span className="font-14">
                      {(
                        (order.option.optionPrice + order.productPrice) *
                        order.quantity
                      ).toLocaleString()}
                      원
                    </span>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td className="order-complate-td  order-complate-line">
                  {/* 배송지 주소 */}
                  <span className="font-14">
                    ({orderInfo.postZonecode}) {orderInfo.postAddr1}{" "}
                    {orderInfo.postAddr2}
                  </span>
                </td>
              </tr>
          ...
          <table className="order-complate-third-table margin-top-20">
            <tbody>
              <tr>
                ...
                <td className="order-complate-td">
                  <span className="font-14">
                    {orderInfo.subtotalAmount?.toLocaleString()}원
                  </span>
                </td>
              </tr>
              <tr>
                ...
                <td className="order-complate-td order-complate-line">
                  <span className="font-14">
                    {orderInfo.shippingAmount?.toLocaleString()}원
                  </span>
                </td>
              </tr>
              <tr>
