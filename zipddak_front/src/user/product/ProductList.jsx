...
export default function ProductList() {
  ...
  const toggleFavorite = async (productIdx) => {
    try {
      ...
      setProductList((prev) =>
        prev.map((p) =>
          p.productIdx === productIdx ? { ...p, favorite: !p.favorite } : p
        )
      );
    } catch (error) {
      console.error(error);
      ...
      <div className="productList-main-div">
        ...
        <div className="cate-list">
          {[
            ...
            >
              <img
                className="p-cate-img"
                src={`/productCateImg/${cate.img}`}
                alt={cate.label}
              />
        ...
        <div className="cate-filter">
          ...
          <div>
            {dataMap[pCateNo]?.map((cate) => (
              <button
                className={
                  middleCateNo === cate.middleCateNo
                    ? "click-middle-cate"
                    : "middle-cate"
                }
                key={cate.middleCateNo}
                onClick={() => setMiddleCateNo(cate.middleCateNo)}
              >
                {cate.name}
              </button>
            ))}
          ...
          <div className="filter-div">
            {filter.map((fil) => (
              <button
                key={fil.filterNo}
                onClick={() => setFilterNo(fil.filterNo)}
                className={
                  filterNo === fil.filterNo
        ...
        >
          {productList.map((product, index) => (
            ...
            >
              <Product
                product={product}
                toggleFavorite={() => toggleFavorite(product.productIdx)}
              />
            </div>
          ))}
        </div>
