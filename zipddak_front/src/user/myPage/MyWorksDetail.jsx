...
export default function MyWorksDetail() {
  ...
  const getEstimates = () => {
    myAxios(token, setToken)
      .get(
        "http://localhost:8080" +
          `/matching/userDetail?matchingIdx=${matchingIdx}`
      )
      .then((res) => {
  ...
  return (
    <div className="mypage-layout">
      ...
      >
        <div style={{ flex: 1 }}>
          <h3 className="mypage-sectionTitle">결제 정보</h3>
          <div className="labelInput-wrapper">
            <label style={{ width: "150px" }}>총 결제 금액</label>
            <p
              style={{
                color: "#FF5833",
                fontWeight: "600",
          ...
          <div className="labelInput-wrapper">
            <label style={{ width: "150px" }}>결제 방법</label>
            <p>{estimate.method}</p>
          </div>
        </div>
        <div style={{ flex: 2 }}>
